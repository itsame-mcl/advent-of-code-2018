...
def compute_score(mobs):
    ...
    while fighting:
        ...
        round = round + 1
    return([score, mobs[0][0], len(mobs)])

elf_power = 3
wins_with_no_loss = False
while not wins_with_no_loss:
    grid = list()
    ...
    for y, line in enumerate(grid):
        for x, square in enumerate(line):
            if (grid[y][x] == "E"):
                mobs.append([grid[y][x],x,y,200,elf_power,True])
                elfs_count = elfs_count + 1
            elif (grid[y][x] == "G"):
                mobs.append([grid[y][x],x,y,200,3,True])
    battle = compute_score(mobs)
    if battle[1] == "E" and battle[2] == elfs_count:
        print(battle[0])
        wins_with_no_loss = True
